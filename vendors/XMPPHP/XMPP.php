...
class XMPPHP_XMPP extends XMPPHP_XMLStream {
	...
	*/
    ...
    /// add
    public function registerNewUser($user_name, $password, $email = NULL, $name = NULL, $server)
    {
        $id = 'reg_' . $this->getID();
    ...
    public function createRoom($server)
    {
        ...
        $id = 'req_' . $this->getID();
        $xml1 = "<iq id='$id' to='{$this->user}@conference.{$server}' type='get'><query xmlns='http://jabber.org/protocol/muc#owner'></query></iq>";
        $this->send($xml1);

        $id= 'req_' . $this->getID();
        $xml = "<iq id='$id' to='{$this->user}@conference.{$server}' type='set'>";
